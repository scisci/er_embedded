...
enum tag_sns_t { KEY_TYPE, SLIDER_TYPE, BANK_TYPE } sns_t;

/* The command type is a received command which has a message type and up to 16
 * commands */
struct command_t {
...
enum {
  ...
  REF_TYPE,
  /* The message contains a single packed 10-bit ref followed by an 8-bit signed
     delta */
  REF_DELTA_TYPE
};
...
// SLIDER_CHANNEL_TYPE	// 2 Bytes that combine to form a 16-bit slider data
// type (3-bit type,3-bit channel,10-bit value)
//};

/* Message types (0-15) */
typedef enum tag_er_msg_t {
  MSG_SNS_UNKNOWN,
  MSG_SNS_STATUS,   // Message type for sending sensor states (on/off, slider
                    // pos)
  MSG_SNS_CHANNEL,  // Message type for sending channels data including
                    // signals/refs
...
#define ER_MIN_RX_BYTES_FOR_EVENT \
  4  // Need Product Flag (2) + Address/Msg + Cmd
#endif
...
/* Number of events in the queue, used externally to check whether to call
 * broadcast */
uint8_t event_count;
