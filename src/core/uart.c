...
{
	return uart_rx_buffer_start ^ (*(volatile uint8_t*)&(uart_rx_buffer_end));		// rx_in modified by interrupt !
}
...
uint8_t uart_unbuffer_rx(void)
{
	uint8_t data;
  	...
  	while( !uart_rx_avail() );								// until at least one byte in
