...
y_line_info_t y_line_info[NUM_Y_LINES]= {
    ...
    FILL_OUT_Y_LINE_INFO(  2u ),
    FILL_OUT_Y_LINE_INFO(  3u ),
	FILL_OUT_Y_LINE_INFO(  4u ),
    FILL_OUT_Y_LINE_INFO(  5u ),
...
#define UPDATE_CHANNEL_REF(ch) 			(ribbon_channel_data[(ch)].reference = qt_measure_data.channel_references[(ch)])

/* Returns the current measured channel delta (ref-sig) */
#define GET_CHANNEL_DELTA(ch) 			(qt_measure_data.channel_references[(ch)]-qt_measure_data.channel_signals[(ch)])
...
#define UPDATE_CHANNEL_DELTA(ch,delta) 	(ribbon_channel_data[(ch)].delta = delta)
...
#define UPDATE_KEY_DELTA(s,d) 	UPDATE_CHANNEL_DELTA(sensors[(s)].from_channel,d)
...
#define CLAMP_INT16_TO_INT8(x) (((int16_t)(x) > (int16_t)(127)) ? (int8_t)(127) : (((int16_t)(x) < (int16_t)(-127)) ? (int8_t)(-127) : (int8_t)(x)))
...
/* Running mode bits 0:rsv,0:rsv,0:rsv,0:rsv,0:rsv,0:rsv,1:dispatch data messages,1:dispatch status messages */
...
------------------------------------------------------------------------------
Purpose :   returns the actual value of the global parameter as it has been set from the last reset
Input   :   (uint8_t)index: parameter index
Output  :   n/a
Notes   :	
============================================================================*/
uint8_t get_mapped_global_param(uint8_t index)
{
	...
	if(index<global_param_count)
	{
		...
		{
			case ER_ADDRESS_PARAM: 			value = ribbon_library_info.address; break;
			case ER_DI_PARAM: 				value = qt_config_data.qt_di; break;
			case ER_MAX_ON_PARAM:			value = qt_config_data.qt_max_on_duration; break;
			case ER_POS_DRIFT_PARAM:		value = qt_config_data.qt_pos_drift_rate; break;
			case ER_NEG_DRIFT_PARAM:		value = qt_config_data.qt_neg_drift_rate; break;
			case ER_RECAL_PARAM:			value = qt_config_data.qt_recal_threshold; break;
			case ER_POS_RECAL_DELAY_PARAM:	value = qt_config_data.qt_pos_recal_delay; break;
			case ER_DRIFT_HOLD_PARAM:		value = qt_config_data.qt_drift_hold_time; break;
			case ER_DATA_INTERVAL_PARAM: 	value = ribbon_library_info.data_interval; break;
			case ER_MEASURE_PERIOD_MS_PARAM:value = ribbon_library_info.measurement_period_ms; break;
...
Input   :   (uint8_t)index: parameter index into mapped array, (uint8_t) value: parameter value
Output  :   n/a
Notes   :	does not change the library until reset
============================================================================*/
...
------------------------------------------------------------------------------
Purpose :   returns the actual value of the sensor parameter as it has been set from the last reset
...
uint8_t get_mapped_sensor_param(uint8_t sensor, uint8_t index)
{
	if(index<sensor_param_count && sensor<ER_MAX_SENSORS)
	{
		...
		{
			case ER_THRESHOLD_PARAM:	return sensors[sensor].threshold;
			case ER_HYSTERESIS1_PARAM:	return (sensors[sensor].type_aks_pos_hyst&0x03);

	#if (NUM_SLIDERS>0)
			case ER_HYSTERESIS2_PARAM: return slider_params[sensors[sensor].index].position_hysteresis;
			case ER_RESOLUTION_PARAM:	return slider_params[sensors[sensor].index].position_resolution;
	#endif	
			default: return 0;
		}
	}
...
Input   :   (uint8_t)sensor: sensor index, (uint8_t)index: parameter index, (uint8_t) value: parameter value
Output  :   n/a
Notes   :	does not change the sensor until reset
============================================================================*/
bool set_mapped_sensor_param(uint8_t sensor, uint8_t index, uint8_t value)
{
	if(index<sensor_param_count && sensor<ER_MAX_SENSORS)
	{
		eeprom_write_byte(&sensor_params[(uint16_t)sensor*ER_MAX_SENSOR_PARAMS+index],value);
		return true;
	}
...
Input   :   (uint8_t)sensor: sensor index, (uint8_t)channel: channel index, (uint8_t) value: parameter value
Output  :   n/a
Notes   :	can only set burst lengths for now
============================================================================*/
...
void ribbon_eeprom_reset(void)
{
	...
	{
		eeprom_write_byte(&sensor_params[i],sensor_param_defaults[i%ER_MAX_SENSOR_PARAMS]);
	}
...
bool ribbon_set_sensor(uint8_t sensor_index, uint8_t sensor_type, uint8_t from, uint8_t to)
{
...
Name    :   data_loop
------------------------------------------------------------------------------
Purpose :   main loop for maximum data acquisition such as calibration or visualization
Input   :   n/a
Output  :   n/a
Notes   :
============================================================================*/
...
uint8_t default_ribbon_status_callback(uint16_t sflag)
{
	...
	if(sflag&QTLIB_STATUS_CHANGE)
	{
		...
		for(i=0;i<sensor_count;i++)
		{
			...
			{
				if(qt_measure_data.channel_signals[(j)] <= qt_measure_data.channel_references[(j)]/2)
				{
#if (RIBBON_DEBUG_MODE==DEBUG_LOOP)
				...
				for(j=0;j<8;j++,s++)
				{
					...
					{	
						if(IS_KEY(s)) 	// KEY
						{
							...
							add_event(BYTE_DATA,s,(qt_measure_data.qt_touch_status.sensor_states[i]>>j)&0x01);
#endif // DEBUG_LOOP
						}
						else 			// SLIDER
						{
							...
							if((qt_measure_data.qt_touch_status.sensor_states[i]>>j)&0x01)
							{
...
Name    :   config_sensor
------------------------------------------------------------------------------
Purpose :   Initializes data structures and calls user function to initialize keys and sliders
Input   :   n/a
Output  :   n/a
...
static void config_sensors(void)
{
	...
	else
	{
		...
		for(i=0;i<loaded_sensor_count;i+=3)
		{
			...
			{
				case 0: // Key
					enable_key(	eeprom_read_byte(&sensor_config[i+1]),
								NO_AKS_GROUP);
					break;
				case 1:
					enable_slider(	eeprom_read_byte(&sensor_config[i+1]),
									eeprom_read_byte(&sensor_config[i+2]),
									NO_AKS_GROUP);
					break;
...
void enable_key(	channel_t channel,
                     	aks_group_t aks_group)
{
	uint8_t threshold, hysteresis;

	if((threshold = eeprom_read_byte(SNS_PARAM_ADR(sensor_count,ER_THRESHOLD_PARAM)))==0xFF)
		threshold = ER_THRESHOLD_DEF;

	if((hysteresis = eeprom_read_byte(SNS_PARAM_ADR(sensor_count,ER_HYSTERESIS1_PARAM)))==0xFF)
...
void enable_slider(  channel_t from_channel,
                        	...
                        	aks_group_t aks_group)
{
	...
	if((resolution = eeprom_read_byte(SNS_PARAM_ADR(sensor_count,ER_RESOLUTION_PARAM)))==0xFF)
		resolution = ER_RESOLUTION_DEF;

	if((pos_hysteresis = eeprom_read_byte(SNS_PARAM_ADR(sensor_count,ER_HYSTERESIS2_PARAM)))==0xFF)
		pos_hysteresis = ER_HYSTERESIS2_DEF;

	qt_enable_slider(from_channel,to_channel,aks_group,
						threshold,hysteresis,resolution,pos_hysteresis);
                        	...
                        	aks_group_t aks_group)
{
	...
	qt_enable_slider(from_channel,to_channel,aks_group,
						12u, 0u, 3u, 0u);//threshold,hysteresis,resolution,pos_hysteresis);
...
static void set_parameters( void )
{
	...
	ribbon_library_info.address		  = eeprom_read_byte(&global_params[ER_ADDRESS_PARAM]);
	ribbon_library_info.data_interval = eeprom_read_byte(&global_params[ER_DATA_INTERVAL_PARAM]);
	ribbon_library_info.config_flags  = eeprom_read_byte(&custom_config);
	ribbon_library_info.measurement_period_ms = eeprom_read_byte(&global_params[ER_MEASURE_PERIOD_MS_PARAM]);

  qt_config_data.qt_di              = eeprom_read_byte(&global_params[ER_DI_PARAM]);
  qt_config_data.qt_neg_drift_rate  = eeprom_read_byte(&global_params[ER_NEG_DRIFT_PARAM]);
  qt_config_data.qt_pos_drift_rate  = eeprom_read_byte(&global_params[ER_POS_DRIFT_PARAM]);
  qt_config_data.qt_max_on_duration = eeprom_read_byte(&global_params[ER_MAX_ON_PARAM]);
  qt_config_data.qt_drift_hold_time = eeprom_read_byte(&global_params[ER_DRIFT_HOLD_PARAM]);
  qt_config_data.qt_recal_threshold = eeprom_read_byte(&global_params[ER_RECAL_PARAM]);
	qt_config_data.qt_pos_recal_delay = eeprom_read_byte(&global_params[ER_POS_RECAL_DELAY_PARAM]);
	
#ifdef _QMATRIX_
	eeprom_read_block((void*)&qt_burst_lengths, (const void*)&ch_burst_lengths, QT_NUM_CHANNELS); 
#endif
}
...
static void notify_init()
{
	...
	if(!eeprom_valid)
		uart_string_P(PSTR("EEPROM INVALID!\r\n"),17);
#endif // RIBBON_DEBUG_ON
}
...
Name    :   ribbon_init
------------------------------------------------------------------------------
Purpose :   initializes clock to correct speed, disables general io pins and sets up uart baud
Input   :   n/a
Output  :   n/a
Notes   :
============================================================================*/
