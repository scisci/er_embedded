...
/* The maximum allocation for slider data chunks, each slider has up to 8
   channels, and each channel can have up to 3 data bytes (delta_ref), each key
   can have 1 channel, there can only be 1 event per sensor so we figure that if
   all of them were slider events there could be ER_MAX_SLIDERS*8 and there
   could be MAX_EVENTS-ER_MAX_SLIDERS more data*/
#define MAX_CHANNEL_DATA                                          \
  ((ER_MAX_SLIDERS) * (ER_MAX_CH_PER_SLIDER)*MAX_CH_DATA_BYTES) + \
      ((MAX_EVENTS - ER_MAX_SLIDERS) * (ER_MAX_CH_PER_KEY)*MAX_CH_DATA_BYTES)
...
#define EVENT_MAX_BUFFER_SIZE \
  (MAX_CHANNEL_DATA > MAX_CMD_DATA ? MAX_CHANNEL_DATA : MAX_CMD_DATA)
...
extern bool set_mapped_sensor_param(uint8_t sensor, uint8_t index,
                                    uint8_t value);
extern bool set_mapped_channel_param(uint8_t sensor, uint8_t channel,
                                     uint8_t value);
...
static uint8_t last_ch_evt =
    0u;  // When adding extra ch data, need to store this
...
void add_global_param_event(uint8_t param_id, uint8_t value) {
  ...
  if (++event_count == MAX_EVENTS) broadcast(MSG_PARAM);
}

void add_sensor_param_event(uint8_t sensor, uint8_t param_id, uint8_t value) {
  events[event_count].id_and_type =
      ((SENSOR_PARAM & 0x03) << 6) | (sensor & 0x3F);
  events[event_count].data[0] = param_id;
  events[event_count].data[1] = value;
...
void add_channel_param_event(uint8_t sensor, uint8_t channel, uint8_t value) {
  events[event_count].id_and_type =
      ((CHANNEL_PARAM & 0x03) << 6) | (sensor & 0x3F);
...
Input   :   uint8_t event_type - the type of status event (right now only
STATUS_EID) uint8_t sensor_id - the sensor id of the event uint8_t data - the
data associated with the event (for a key, 1 or 0, for a slider 1-255 or 0)
Output  :   n/a
Notes   :   n/a
============================================================================*/
void add_event(uint8_t event_type, uint8_t sensor_id, uint8_t data) {
  ...
  if (++event_count == MAX_EVENTS) broadcast(MSG_SNS_STATUS);
}
...
Purpose :   adds a channel data event to the event queue meaning that it will be
                        followed by 1 or more channel data blocks when
broadcast() is called, data events contain references and deltas used in
software calibration and debugging Input   :   uint8_t sensor_id - the sensor id
of the event Output  :   n/a Notes   :   Data events cannot be combined with
status events in the same broadcast! Directly following any call to
add_slider_data_event with a channel data tyep, one or more calls to
add_slider_channel_data must be called to fill it with channel data
============================================================================*/
void add_ch_data_event(uint8_t sensor_id) {
  // If the queue is about to overflow then dispatch it (this shouldn't happen)
  if (event_count + 1 == MAX_EVENTS) broadcast(MSG_SNS_CHANNEL);
...
void broadcast_config(void) {
  for (event_count = 0; event_count < sensor_count; event_count++) {
    events[event_count].id_and_type =
        ((IS_KEY(event_count) ? KEY_TYPE : SLIDER_TYPE) << 2) | WORD_DATA;
...
Input   :   uint8_t data_type - the type of data event (delta, reference,
delta/ref) (0-3) uint8_t channel - the absolute channel id of this channel
Output  :   n/a
Notes   :   n/a
============================================================================*/
void add_ch_data(uint8_t data_type, uint8_t channel) {
  ...
  event_buffer[ch_data_count] =
      ((data_type & 3) << 6) |
      (((channel - sensors[last_ch_idx].from_channel) & 0x1F) << 2);
  ...
  if (data_type == DELTA_TYPE) {
    ...
    event_buffer[ch_data_count] |=
        (ribbon_channel_data[channel].reference >> 8) & 3;
    // Put the remaining reference value in the second byte
    event_buffer[ch_data_count + 1] =
        (ribbon_channel_data[channel].reference & 0xFF);
    // Update the byte count of the last channel event
    events[last_ch_evt].data[1] += 2;
    // Increment the channel data pointer
    ch_data_count += 2;
...
Input   :   uint8_t cmd - the command, bool valid - whether the command
completed successfully Output  :   n/a Notes   :   n/a
============================================================================*/
...
void broadcast(uint8_t message_type) {
...
#if ((RIBBON_DEBUG_MODE == DEBUG_EVENT) || \
     (RIBBON_DEBUG_MODE == DEBUG_BROADCAST))
  char str[4];
  uart_string("\r\nBroadcast(", 12);
  ...
  // Send the 4-bit address combined with the 4-bit message type (in this case,
  // sensor data)
  uart_byte(((ribbon_library_info.address & 0xF) << 4) | (message_type & 0xF));
  check_sum +=
      ((ribbon_library_info.address & 0xF) << 4) | (message_type & 0xF);
  ...
  if (message_type ==
      MSG_SNS_STATUS)  // Status messages are for performance mode
  {                    // Minimal bytes is best
    ...
    for (i = 0; i < event_count; i++) {
      ...
      // Determine the event size  via data[1] and add it to the id_and_type
      // header
  ...
  } else if ((message_type == MSG_MODE) || (message_type == MSG_CMD) ||
  ...
  switch (last_cmd.command_type) {
    case CMD_DUMP_ALL_PARAMS:
      ...
      for (s = 0; s < sensor_count; s++) {
        ...
        for (i = sensors[s].from_channel; i <= sensors[s].to_channel; i++)
          add_channel_param_event(
              s, i - sensors[s].from_channel,
        ...
        switch (event_buffer[0] >> 6) {
...
#endif
            command_valid =
                set_mapped_global_param(event_buffer[1], event_buffer[2]);
            break;
          ...
          case SENSOR_PARAM:
            ...
            command_valid = set_mapped_sensor_param(
                event_buffer[0] & 0x3F, event_buffer[1], event_buffer[2]);
            break;
    ...
    case CMD_SET_SENSOR:
      if (last_cmd.command_count >= 4) {
        command_valid = ribbon_set_sensor(event_buffer[0], event_buffer[1],
...
void parse_rx_event(void) {
  ...
  rx_timeout = current_time_ms_touch +
               50u;  // Set a timeout so we don't neglect the rest of the app

  rx = uart_unbuffer_rx();  // Grab the top char, (make sure to check uart_avail
                            // before)
  ...
  do {
    ...
    if (!(prs_flags & PRS_ADDRS_FLAG))  // Check if we alread got the address
    {
      ...
      if (rx == ER_GLOBAL_ADDRESS ||
          (rx < ER_MAX_ADDRESSES &&
           ribbon_library_info.address == ER_GLOBAL_ADDRESS) ||
        ...
        rx_timeout += 25u;  // Since the message is for us,
                            // wait a bit longer
        while (!uart_rx_avail() && current_time_ms_touch < rx_timeout)
          ;  // We got a matching address
    ...
    if (!(prs_flags & PRS_PRID2_FLAG))  // Look for the second sync flag
    {
      if (rx == ribbon_library_info.pid[1]) {
        ...
        while (!uart_rx_avail() &&  // Wait until we get the next
               current_time_ms_touch < rx_timeout)
          ;  // char or reach the max timeout
    ...
    if (!(prs_flags & PRS_CMDTP_FLAG))  // See if we parse the command and type
    {
      ...
      if (((RIBBON_MODE !=
            ER_CMD_LOOP) &&  // Command is only good if we are in command mode
...
#if (RIBBON_DEBUG_MODE == DEBUG_RX)
  if (small_buffer_pos > 2) print_small_buffer = 1;
