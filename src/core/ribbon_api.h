...
#ifndef DEVICE_ADR
	#define DEVICE_ADR 2u	// Unique Address assigned to each device (reprogrammable)
							// (0-15s, broadcasted)
#endif
...
//#define DYNAMIC_CLOCKING		// Changes clock rates to 4mhz when sampling and 16mhz during serial comms
//#define DYNAMIC_CLOCKING_NOFF 	// Makes the program start at 4mhz and only increase to 16mhz for serial
#define STATIC_CLOCKING_NOFF	// Specifies that the static clock run at low speed normally
...
#define DEF_GLOBAL_PARAM_MAP(count,...) uint8_t global_param_count = count; \
										uint8_t global_param_map[count] = { __VA_ARGS__ }
										
										
#define DEF_SENSOR_PARAM_MAP(count,...) uint8_t sensor_param_count = count; \
...
// Check if there is a change in a specific byte of the sensor state array and return the bits that have changed
#define STATUS_BYTE_CHANGE(i) 	(sensor_states[(i)]^qt_measure_data.qt_touch_status.sensor_states[(i)])

// Updates the local copy of the sensor states, by copying from qt_measure_data */
#define UPDATE_STATUS_BYTE(i) 	(sensor_states[(i)]=qt_measure_data.qt_touch_status.sensor_states[(i)])

// Checks if the position of a sider has changed since last time (slider index, not sensor)
#define SLIDER_POS_CHANGE(s) 	(slider_positions[(s)] != qt_measure_data.qt_touch_status.rotor_slider_values[(s)])

// Updates the local copy of the slider position by copying from qt_measure_data 
#define UPDATE_SLIDER_POS(s)	(slider_positions[(s)]=qt_measure_data.qt_touch_status.rotor_slider_values[(s)])
...
#define SET_RIBBON_MODE(mode) if(((mode)&0xF)<ER_MAX_LOOP){ mode_flags &= 0xF0; mode_flags |= ((mode)&0xF); }
...
{
	int8_t delta;			// Holds the last read delta of this channel (clamped to -127 +127)
...
bool ribbon_set_sensor(uint8_t sensor_index, uint8_t sensor_type, uint8_t from_ch, uint8_t to_ch);
